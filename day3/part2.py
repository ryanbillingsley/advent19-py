...
def move(start_pos, end_pos, points):
    ...
    if start_pos[0] < end_pos[0]:
        ...
        move_x = -1
    if move_x != 0:
        for i in range(start_pos[0] + move_x, end_pos[0] + move_x, move_x):
            points.append((i, start_pos[1]))
    ...
    if start_pos[1] < end_pos[1]:
        ...
        move_y = -1
    if move_y != 0:
        for i in range(start_pos[1] + move_y, end_pos[1] + move_y, move_y):
            points.append((start_pos[0], i))
...
commons = set(points[0]).intersection(points[1])
...
# find the index for the common values, use that index to calculate total number of steps
step_distance = lambda common: (points[0].index(common) + 1) + (points[1].index(common) + 1)
distances = map(step_distance ,commons)

print(min(list(distances)))
# for common in commons:
#     step1 = points[0].index(common) + 1
#     step2 = points[1].index(common) + 1
#     print(f'Distance: [{step1}:{step2}] {common}')